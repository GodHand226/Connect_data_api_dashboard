...
import { API_SERVER, API_ENDPOINT } from "@/config/constant";
...
export function Tables() {
  ...
  const getdata = async (body) => {
    ...
    const result = await axios.post(API_ENDPOINT, body, { headers: header });
    return result.data;
  };
  const getAbbr = () => {
    var abbr = [];
  ...
  const SearchClicked = async () => {
    let curl = `curl -X 'POST' '${API_ENDPOINT}' -H 'accept: application/json' -H 'access_token: ${key}' -H 'Content-Type: application/json'`;
    setCurl(curl);
    setUrl(API_ENDPOINT);
    var abbr = getAbbr();
    let body = {
